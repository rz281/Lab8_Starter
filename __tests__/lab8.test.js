describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let cartbutton = await shadowroot.asElement().$('button');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let index = 1; index < 20; index++) {
      let shadowroot = await prodItem[index].getProperty('shadowRoot');
      let cartbutton = await shadowroot.asElement().$('button');
      await cartbutton.click();
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let index = 0; index < 20; index++) {
      ...
      let cartbutton = await shadowroot.asElement().$('button');
      let buttonText = await cartbutton.getProperty('innerText');
      let actualText = buttonText['_remoteObject'].value;
      expect(actualText).toBe('Remove from Cart');
      
    }
    const cartCount = await page.$('#cart-count');
    let count = await cartCount.getProperty('innerText');
    let actualCount = count['_remoteObject'].value;
    expect(parseInt(actualCount)).toBe(20);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    expect(actualCart).toBe(expectedCart);
  });
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for (let i = 0; i < items.length; i++){
      let shadowRoot = await items[i].getProperty('shadowRoot');
      let btn = await shadowRoot.$('button');
      await btn.click();
    }
    let cartCount = await page.$('#cart-count');
    let innerText = await cartCount.getProperty('innerText');
    let actualCount = innerText['_remoteObject'].value;
    expect(parseInt(actualCount)).toBe(0);
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < items.length; i++){
      ...
      let actualText = innerText['_remoteObject'].value;
      expect(actualText).toBe('Add to Cart');
    }
